...
export class GraphScatterLigandComponent implements OnInit {
  ...
  chartClick(param){
    ...
    this.rest.getData(`chembl/compound-structures/${molId}/?exclude[]=*&include[]=canonical_smiles`)
      .subscribe(data=>{
        const smile=data['compound_structures'].canonical_smiles;
        this.openDialog(smile,molName);
      })

  }

  openDialog(smile:string,name:string):void{
    let dialogRef=this.dialog.open(CompoundCardComponent,{
      width:'250px',
      height:'300px',
      data:{smile:smile,chembl_name:name}
    });
